...
# python3.6 splitprob.py /scratch/mmarcheg/lumi_data/Run300806.root 7 5 -o -s Run99 -l
...
lumibinned = False
if (sys.argv[7] == "-l") | (sys.argv[7] == "-lumi"):
	lumibinned = True
...
if lumibinned == True:
	nbins_lumi = 10
	lumi_bins = np.quantile(df_grid['instaLumi'].values, np.linspace(0, 1, nbins_lumi + 1))
	lumi_n, lumi_bins, patches = plt.hist(df_grid_full['instaLumi'], bins=nbins_lumi, facecolor="green", ec="black")
	plt.close()
...
for ladder in ["inner", "outer"]:
	...
	df_grid_full_ladder = sp.select_ladder(df_grid_full, ladder)
	print("FULL")
	df_grid_broken_ladder = sp.select_ladder(df_grid_broken, ladder)
	print("BROKEN")
	...
	#rows_max = df_grid_full['rows'].max()
		bins = [100, 100, 20, df_grid_full_ladder['bx'].max() - df_grid_full_ladder['bx'].min(), 50]
		rows_max = 8
		rows = range(1, rows_max+1)
		for nrows in rows:
			df_grid_full_rows = pd.DataFrame()
			df_grid_broken_rows = pd.DataFrame()
			df_grid_full_rows = sp.select_rows(df_grid_full_ladder, nrows)
			print("FULL")
			df_grid_broken_rows = sp.select_rows(df_grid_broken_ladder, nrows)
			print("BROKEN")
