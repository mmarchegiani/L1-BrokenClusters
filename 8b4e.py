...
plot_dir = "../ntuplesPixel/plots/8b4e/"
#os.mkdir(plot_dir)
	...
	for ladder in ["inner", "outer"]:
		...
		for (j, item) in enumerate(n_broken_sum_list):
			...
			if n_broken_sum_list[j] != [0]*(len(bins[0]) - 1):
				sp.splitprob_n(n_full_sum_list[j], n_broken_sum_list[j], nfull, nbroken, ladder, varname="global_eta", luminame=luminame[j], output=True, plot_dir=plot_dir)
